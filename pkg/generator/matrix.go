...
func (m *MatrixGenerator) getParams(appSetBaseGenerator argoprojiov1alpha1.ApplicationSetNestedGenerator, appSet *argoprojiov1alpha1.ApplicationSet, params map[string]interface{}) ([]map[string]interface{}, error) {
	...
	if matrixGen != nil && !appSet.Spec.ApplyNestedSelectors {
		...
		if foundSelector {
			log.Warn().Msgf("AppSet '%v' defines selector on nested matrix generator's generator without enabling them via 'spec.applyNestedSelectors', ignoring nested selectors", appSet.Name)
		}
	}
	...
	if mergeGen != nil && !appSet.Spec.ApplyNestedSelectors {
		...
		if foundSelector {
			log.Warn().Msgf("AppSet '%v' defines selector on nested merge generator's generator without enabling them via 'spec.applyNestedSelectors', ignoring nested selectors", appSet.Name)
		}
	}
