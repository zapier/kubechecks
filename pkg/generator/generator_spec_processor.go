...
func Transform(requestedGenerator argoprojiov1alpha1.ApplicationSetGenerator, allGenerators map[string]Generator, baseTemplate argoprojiov1alpha1.ApplicationSetTemplate, appSet *argoprojiov1alpha1.ApplicationSet, genParams map[string]interface{}) ([]TransformResult, error) {
	...
	for _, g := range generators {
		...
		if err != nil {
			log.Error().Err(err).Msgf("error generating params, generator: %+v", g)
			if firstError == nil {
				firstError = err
			}
			continue
		...
		if len(genParams) != 0 {
			...
			if err != nil {
				log.Error().Err(err).Msgf("error interpolating params for generator. genParams: %+v", genParams)
				if firstError == nil {
					firstError = err
				}
				continue
		...
		if err != nil {
			log.Error().Err(err).Msgf("error generating params generator. generator: %+v", g)
			if firstError == nil {
				firstError = err
			}
			continue
		...
		for _, param := range params {
			...
			if err != nil {
				log.Error().Err(err).Msgf("error flattening params. generator: %+v", g)
				if firstError == nil {
					firstError = err
				}
				continue
...
func InterpolateGenerator(requestedGenerator *argoprojiov1alpha1.ApplicationSetGenerator, params map[string]interface{}, useGoTemplate bool, goTemplateOptions []string) (argoprojiov1alpha1.ApplicationSetGenerator, error) {
	...
	if err != nil {
		log.Error().Err(err).Msgf("error interpolating generator with other generator's parameter. interpolatedGenerator: %+v", interpolatedGenerator)
